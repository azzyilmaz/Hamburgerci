...
namespace Hamburgerci.Models
{
    public class Siparis
    {
        ...
        public override string ToString()
        {
            if (Extralar.Count < 1)
            {
                return string.Format("{0} Menü X {1} Adet, {2} Boy, Toplam : {3}", Menu.MenuAdi, Adet, Boyut.ToString(), ToplamTutar.ToString("C2"));
            }
            else
            {
                ...
                return string.Format("{0} Menü X {1} Adet, {2} Boy, Ex: {3}, Toplam : {4}", Menu.MenuAdi, Adet, Boyut.ToString(), extraMalzemeler, ToplamTutar.ToString("C2"));
            }
        }
    }
}
